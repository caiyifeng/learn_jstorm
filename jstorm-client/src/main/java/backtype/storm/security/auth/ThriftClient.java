...
public class ThriftClient {
    ...
    /**
     * 连接到登记在/nimbus_master下的leader的nimbus.thirft.port端口
     */
    ...
    /**
     * ITransportPlugin有SimpleTransportPlugin和DigestSaslTransportPlugin最终实现的子类
     * 具体使用那一个类会在storm.thrift.transport里面配置
     * 前者无需用户登录信息，后者的话需要在环境变量java.security.auth.login.config里面配置文件地址
     * 最后调用ITransportPlugin的connect方法与nimbus leader连接
     */
    protected void connect(Map storm_conf, Integer timeout) throws Exception {
        try {
